...
public class TripService {
	...
	// 26 Explorers have a finder in which they can specify some search...
	public Collection<Trip> findTripsExplorer(@Valid final Finder finder) {

		// Inicialize variables
		final Collection<Trip> res;
		final String criteria = finder.getSearch();
		DateTime maxDate;
		DateTime minDate;
		Double minPrice;
		Double maxPrice;
		final Collection<Trip> res1 = new ArrayList<Trip>();

		// Catch if some value is null
		if (finder.getMaxDate() == null)
			maxDate = new DateTime(9999, 1, 1, 0, 0, 0, DateTimeZone.UTC);
		else
			maxDate = new DateTime(finder.getMaxDate());
		if (finder.getMinDate() == null)
			minDate = new DateTime(0000, 1, 1, 0, 0, 0, DateTimeZone.UTC);
		else
			minDate = new DateTime(finder.getMinDate());
		...
		// Search in db trips by a keyword
		res = this.tripRepository.findTripByKeyword(criteria);

		for (final Trip t : res)
			if (t.getPrice() >= minPrice && t.getPrice() <= maxPrice && new DateTime(t.getStartTrip()).compareTo(minDate) >= 0 && new DateTime(t.getStartTrip()).compareTo(maxDate) <= 0)
				res1.add(t);
